...
function BuyRequestsPage() {
  ...
  const handleViewDetails = async (request) => {
    try {
      ...
      // Load full details
      const response = await fetch(`${BACKEND_URL}/api/public/buy-requests/${request.id}`, {
        headers: user ? {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        } : {}
      });
  ...
  const handleSendOffer = async (request) => {
    ...
    if (!user.roles?.includes('seller')) {
      alert('You need to be registered as a seller to send offers. Please update your profile.');
      return;
    }
    
    // Load full request details for offer
    try {
      const response = await fetch(`${BACKEND_URL}/api/public/buy-requests/${request.id}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
      ...
      console.error('Error loading request for offer:', error);
      setSelectedRequest(request);
      setShowOfferModal(true);
    }
  };

  // Handle accept offer and redirect to checkout
  const handleAcceptOffer = async (request, offer) => {
    try {
      ...
      navigate(`/checkout/${result.order_group_id}`);
  ...
  const getTimeRemaining = (deadlineAt) => {
    ...
    if (days > 2) return { text: `${days}d ${hours}h`, color: 'text-yellow-600' };
    if (days > 0) return { text: `${days}d ${hours}h`, color: 'text-orange-600' };
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-green-50 to-emerald-100">
        <div className="container mx-auto px-4 py-8">
          <div className="text-center py-12">
            ...
            <p className="text-emerald-600">Loading buy requests...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-green-50 to-emerald-100">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          <p className="text-emerald-600">
            {requests.length} active requests found
          </p>
        </div>

        {/* Results Grid - Enhanced with more details */}
        {requests.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {requests.map(request => {
              const timeRemaining = getTimeRemaining(request.deadline_at);
              return (
                <Card key={request.id} className="bg-white border-emerald-200 hover:shadow-lg transition-all duration-200 cursor-pointer">
                  <CardHeader className="pb-3">
                    <div className="flex justify-between items-start mb-3">
                      <CardTitle className="text-lg font-medium text-emerald-900 flex-1">
                        WANTED: {request.title || `${request.species} Needed`}
                      </CardTitle>
                      <Badge 
                        variant={request.has_target_price ? 'default' : 'secondary'} 
                        className={request.has_target_price ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}
                      >
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <span className="ml-2 font-medium text-emerald-700 capitalize">{request.species}</span>
                        </div>
                        <div>
                          <span className="text-gray-500">Type:</span>
                          <span className="ml-2 font-medium capitalize">{request.product_type || 'General'}</span>
                        </div>
                      </div>

                      {/* Target Price - Enhanced Display */}
                      {request.target_price && (
                        <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                          <div className="flex items-center justify-between">
                            <span className="text-sm text-green-700 font-medium">Target Budget:</span>
                            <span className="text-lg font-bold text-green-800">
                              R{request.target_price} <span className="text-sm font-normal">per {request.unit}</span>
                            </span>
                          </div>
                          <div className="text-xs text-green-600 mt-1">
                            Total budget: R{(request.target_price * request.qty).toLocaleString()}
                          </div>
                        </div>
                      )}
                  ...
                  <CardContent className="pt-0">
                    ...
                    {request.notes_excerpt && (
                      <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                        <p className="text-sm text-gray-700 leading-relaxed">
                          {request.notes_excerpt.substring(0, 120)}
                          {request.notes_excerpt.length > 120 ? '...' : ''}
                        </p>
                      </div>
                    )}
                    ...
                    <div className="flex justify-between items-center mb-4">
                      <span className={`text-sm flex items-center gap-1 font-medium ${timeRemaining.color}`}>
                    ...
                    <div className="flex gap-2">
                      ...
                      {/* Show different buttons based on user role and request ownership */}
                      {user && user.roles?.includes('buyer') && request.buyer_id === user.id && request.offers_count > 0 ? (
                        <Button 
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleViewDetails(request); // This will show offers in detail modal
                          }}
                        >
                          <Package className="h-4 w-4 mr-1" />
                          View Offers ({request.offers_count})
                        </Button>
                      ) : (
                        <Button 
                          size="sm"
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-emerald-900 mb-2">No buy requests found</h3>
            <p className="text-emerald-600 mb-4">
              Check back later for new livestock purchase opportunities.
            </p>
            {user && user.roles?.includes('buyer') && (
              <Button 
                onClick={() => navigate('/create-buy-request')}
        ...
        {/* Offers Modal for Buyers */}
        {user && user.roles?.includes('buyer') && selectedRequest && selectedRequest.offers_count > 0 && (
          <ViewOffersModal 
            request={selectedRequest}
            user={user}
            ...
            onAcceptOffer={(offer) => {
              // Handle accept offer and redirect to checkout
              handleAcceptOffer(selectedRequest, offer);
            }}
          />
        )}
...
function BuyerOffersInbox() {
  ...
  const loadOffers = async () => {
    try {
      setLoading(true);
      // Get all buy requests by this buyer first
      const requestsResponse = await fetch(`${BACKEND_URL}/api/buy-requests/my-requests`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
      ...
      for (const request of myRequests) {
        try {
          const offersResponse = await fetch(`${BACKEND_URL}/api/buy-requests/${request.id}/offers`, {
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
          });
          ...
          if (offersResponse.ok) {
            ...
            const requestOffers = (offersData.items || []).map(offer => ({
              ...offer,
              request: request,
              request_title: request.title || `${request.species} Request`
            }));
      ...
      allOffers.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
      setOffers(allOffers);
      setError(null);
  ...
  const handleAcceptOffer = async (offer) => {
    try {
      const response = await fetch(`${BACKEND_URL}/api/buy-requests/${offer.request.id}/offers/${offer.id}/accept`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-green-50 to-emerald-100">
        <div className="container mx-auto px-4 py-8">
          <div className="text-center py-12">
            <div className="animate-spin h-8 w-8 border-4 border-emerald-500 border-t-transparent rounded-full mx-auto mb-4"></div>
            <p className="text-emerald-600">Loading your offers...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-green-50 to-emerald-100">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-emerald-900">Offers Inbox</h1>
            <p className="text-emerald-700 mt-1">
            ...
            <CardContent className="pt-6">
              <div className="text-center">
                <div className="text-2xl font-bold text-red-600">
                  {offers.filter(o => new Date(o.validity_expires_at) <= new Date()).length}
                </div>
        ...
        {offers.length > 0 ? (
          <div className="space-y-4">
            {offers.map((offer, index) => {
              ...
              return (
                <Card key={offer.id} className={`border-2 hover:shadow-lg transition-all duration-200 ${
                  offer.status === 'pending' ? 'border-emerald-200 bg-white' :
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center gap-2">
                          ...
                          <h4 className="font-medium text-lg">{offer.request_title}</h4>
                        </div>
                        <div className="text-sm space-y-1">
                          <div className="flex justify-between">
                            <span className="text-gray-600">Species:</span>
                            <span className="font-medium">{offer.request.species}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Requested:</span>
                            <span className="font-medium">{offer.request.qty} {offer.request.unit}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Location:</span>
                            <span className="font-medium">{offer.request.province}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      {/* Offer Details */}
                      <div className="space-y-3">
                        <h5 className="font-medium text-emerald-900">Offer Details</h5>
                        <div className="text-sm space-y-1">
                          <div className="flex justify-between">
                            <span className="text-gray-600">Quantity:</span>
                            <span className="font-medium">{offer.qty} {offer.request.unit}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Unit Price:</span>
                            <span className="font-medium">R{unitPrice.toFixed(2)}</span>
                      ...
                      {/* Seller Info */}
                      <div className="space-y-3">
                        <h5 className="font-medium text-emerald-900">Seller Information</h5>
                        <div className="text-sm space-y-1">
                          <div className="flex justify-between">
                            <span className="text-gray-600">Seller:</span>
                            <span className="font-medium">{offer.seller_name || 'Anonymous'}</span>
