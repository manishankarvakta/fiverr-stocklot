...
export function isWithinServiceArea(area, buyer) {
  if (area.mode === 'RADIUS') {
    if (!buyer.latlng) {
...
export function checkDeliveryPolicy(serviceArea, sellerCountry, buyer) {
...
export function getGeofenceReasonText(reason) {
  switch (reason) {
